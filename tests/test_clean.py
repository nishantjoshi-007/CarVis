# Tests for the cleaning rules. Every case is a real defect in the source CSV,
# so this doubles as executable documentation of the data's problems.
...
# A value that was never real becomes NULL, never a substitute number.
...
class TestImplausibleValuesAreKeptAndFlagged:
    # Real-but-extreme is not corrupt: keep it, raise a flag
    ...
    def test_high_price_is_flagged(self):
        value, suspect, _ = parse_price("26307500")
...
class TestEveryRuleToleratesEmptyInput:
    # A parser raising on missing input would kill the entire load
